...
void VulkanBase::createFrameBuffers() {
    ...
    for (size_t i = 0; i < swapChainManager->getSwapChainImageViews().size(); i++) {
        ...
        std::array<VkImageView, 2> attachments = {
            swapChainManager->getSwapChainImageViews()[i], // Swap chain image view
            depthImageView // Depth image view (Assuming you have already created this)
        };
...
void VulkanBase::createDepthResources()
{
    ...
    createImage(swapChainManager->getSwapChainExtent().width, swapChainManager->getSwapChainExtent().height,1, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
